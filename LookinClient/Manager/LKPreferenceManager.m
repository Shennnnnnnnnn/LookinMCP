...
NSString *const NotificationName_DidChangeSectionShowing =
    @"NotificationName_DidChangeSectionShowing";
...
static NSString *const Key_PreferredExportCompression =
    @"preferredExportCompression";
...
static NSString *const Key_MCPServerEnabled = @"mcpServerEnabled";
static NSString *const Key_MCPServerPort = @"mcpServerPort";

@interface LKPreferenceManager ()

@property(nonatomic, strong)
    NSMutableDictionary<LookinAttrSectionIdentifier, NSNumber *>
        *storedSectionShowConfig;
...
- (instancetype)init {
  if (self = [super init]) {
    _previewScale =
        [LookinDoubleMsgAttribute attributeWithDouble:LKInitialPreviewScale];
    _previewDimension = [LookinIntegerMsgAttribute
        attributeWithInteger:LookinPreviewDimension3D];
    _measureState =
        [LookinIntegerMsgAttribute attributeWithInteger:LookinMeasureState_no];
    ...
    NSInteger prevClientVersion =
        [userDefaults integerForKey:Key_PreviousClientVersion];
    if (prevClientVersion != LOOKIN_CLIENT_VERSION) {
      //            clientVersionHasChanged = YES;
      [[NSUserDefaults standardUserDefaults]
          setInteger:LOOKIN_CLIENT_VERSION
              forKey:Key_PreviousClientVersion];
    }

    NSNumber *obj_showOutline = [userDefaults objectForKey:Key_ShowOutline];
    if (obj_showOutline != nil) {
      _showOutline = [LookinBOOLMsgAttribute
          attributeWithBOOL:[obj_showOutline boolValue]];
    } else {
      _showOutline = [LookinBOOLMsgAttribute attributeWithBOOL:YES];
      [userDefaults setObject:@(YES) forKey:Key_ShowOutline];
    }
    [self.showHiddenItems subscribe:self
                             action:@selector(_handleShowOutlineDidChange:)
                      relatedObject:nil];

    NSNumber *obj_showHiddenItems =
        [userDefaults objectForKey:Key_ShowHiddenItems];
    if (obj_showHiddenItems != nil) {
      _showHiddenItems = [LookinBOOLMsgAttribute
          attributeWithBOOL:[obj_showHiddenItems boolValue]];
    } else {
      _showHiddenItems = [LookinBOOLMsgAttribute attributeWithBOOL:NO];
      [userDefaults setObject:@(NO) forKey:Key_ShowHiddenItems];
    }
    [self.showHiddenItems subscribe:self
                             action:@selector(_handleShowHiddenItemsChange:)
                      relatedObject:nil];
    ...
    NSNumber *obj_doubleClickBehavior =
        [userDefaults objectForKey:Key_DoubleClickBehavior];
    if (obj_doubleClickBehavior) {
      ...
      [userDefaults setObject:@(_doubleClickBehavior)
                       forKey:Key_DoubleClickBehavior];
    }
    ...
    zInterspaceValue = MAX(MIN(zInterspaceValue, LookinPreviewMaxZInterspace),
                           LookinPreviewMinZInterspace);
    _zInterspace =
        [LookinDoubleMsgAttribute attributeWithDouble:zInterspaceValue];
    [self.zInterspace subscribe:self
                         action:@selector(_handleZInterspaceDidChange:)
                  relatedObject:nil];

    NSNumber *obj_appearanceType =
        [userDefaults objectForKey:Key_AppearanceType];
    ...
    NSNumber *obj_expansionIndex =
        [userDefaults objectForKey:Key_ExpansionIndex];
    ...
    NSNumber *obj_mcpServerEnabled =
        [userDefaults objectForKey:Key_MCPServerEnabled];
    if (obj_mcpServerEnabled != nil) {
      _mcpServerEnabled = [obj_mcpServerEnabled boolValue];
    } else {
      _mcpServerEnabled = NO;
      [userDefaults setObject:@(_mcpServerEnabled) forKey:Key_MCPServerEnabled];
    }

    NSNumber *obj_mcpServerPort = [userDefaults objectForKey:Key_MCPServerPort];
    if (obj_mcpServerPort != nil) {
      _mcpServerPort = [obj_mcpServerPort integerValue];
    } else {
      _mcpServerPort = 47199;
      [userDefaults setObject:@(_mcpServerPort) forKey:Key_MCPServerPort];
    }
    ...
    NSNumber *obj_syncConsoleTarget =
        [userDefaults objectForKey:Key_SyncConsoleTarget];
    if (obj_syncConsoleTarget != nil) {
      _syncConsoleTarget = [obj_syncConsoleTarget boolValue];
    } else {
      _syncConsoleTarget = YES;
      [userDefaults setObject:@(_syncConsoleTarget)
                       forKey:Key_SyncConsoleTarget];
    }

    NSNumber *obj_freeRotation = [userDefaults objectForKey:Key_FreeRotation];
    if (obj_freeRotation != nil) {
      _freeRotation =
          [LookinBOOLMsgAttribute attributeWithBOOL:obj_freeRotation.boolValue];
    } else {
      _freeRotation = [LookinBOOLMsgAttribute attributeWithBOOL:YES];
      [userDefaults setObject:@(_freeRotation.currentBOOLValue)
                       forKey:Key_FreeRotation];
    }
    [self.freeRotation subscribe:self
                          action:@selector(_handleFreeRotationDidChange:)
                   relatedObject:nil];

    NSNumber *obj_fastMode = [userDefaults objectForKey:Key_FastMode];
    if (obj_fastMode != nil) {
      _fastMode =
          [LookinBOOLMsgAttribute attributeWithBOOL:obj_fastMode.boolValue];
    } else {
      _fastMode = [LookinBOOLMsgAttribute attributeWithBOOL:NO];
      [userDefaults setObject:@(_fastMode.currentBOOLValue)
                       forKey:Key_FastMode];
    }
    [self.fastMode subscribe:self
                      action:@selector(_handleFastModeDidChange:)
               relatedObject:nil];

    self.storedSectionShowConfig =
        [[userDefaults objectForKey:Key_SectionsShow] mutableCopy];
    ...
    NSNumber *obj_preferredExportCompression =
        [userDefaults objectForKey:Key_PreferredExportCompression];
    if (obj_preferredExportCompression != nil) {
      _preferredExportCompression =
          [obj_preferredExportCompression doubleValue];
    } else {
      /// 这里的默认值需要在 LKExportAccessory.m 里定义的选项里面
      _preferredExportCompression = .5;
      [userDefaults setObject:@(_preferredExportCompression)
                       forKey:Key_PreferredExportCompression];
    }

    _receivingConfigTime_Color =
        [userDefaults doubleForKey:Key_ReceivingConfigTime_Color];
    _receivingConfigTime_Class =
        [userDefaults doubleForKey:Key_ReceivingConfigTime_Class];
  }
  return self;
}
...
- (void)_handleShowOutlineDidChange:(LookinMsgActionParams *)param {
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(param.boolValue)
                                              forKey:Key_ShowOutline];
  }
}
...
- (void)setEnableReport:(BOOL)enableReport {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(enableReport)
                                              forKey:Key_EnableReport];
  }
...
- (void)setRgbaFormat:(BOOL)rgbaFormat {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(rgbaFormat)
                                              forKey:Key_RgbaFormat];
  }
}
...
- (void)setDoubleClickBehavior:(LookinDoubleClickBehavior)doubleClickBehavior {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(doubleClickBehavior)
                                              forKey:Key_DoubleClickBehavior];
  }
}
...
- (void)setAppearanceType:(LookinPreferredAppeanranceType)appearanceType {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(appearanceType)
                                              forKey:Key_AppearanceType];
  }
}
...
- (void)setExpansionIndex:(NSInteger)expansionIndex {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(expansionIndex)
                                              forKey:Key_ExpansionIndex];
  }
}

- (void)setMcpServerEnabled:(BOOL)mcpServerEnabled {
  if (_mcpServerEnabled == mcpServerEnabled) {
    return;
  }
  _mcpServerEnabled = mcpServerEnabled;
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(mcpServerEnabled)
                                              forKey:Key_MCPServerEnabled];
  }
}

- (void)setMcpServerPort:(NSInteger)mcpServerPort {
  if (_mcpServerPort == mcpServerPort) {
    return;
  }
  _mcpServerPort = mcpServerPort;
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(mcpServerPort)
                                              forKey:Key_MCPServerPort];
  }
}
...
- (void)setImageContrastLevel:(NSInteger)imageContrastLevel {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(imageContrastLevel)
                                              forKey:Key_ContrastLevel];
  }
}
...
- (void)setCollapsedAttrGroups:(NSArray<NSNumber *> *)collapsedAttrGroups {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:collapsedAttrGroups
                                              forKey:Key_CollapsedGroups];
  }
}
...
- (void)setPreferredExportCompression:(CGFloat)preferredExportCompression {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults]
        setObject:@(preferredExportCompression)
           forKey:Key_PreferredExportCompression];
  }
}
...
- (void)setSyncConsoleTarget:(BOOL)syncConsoleTarget {
  ...
  if (self.shouldStoreToLocal) {
    [[NSUserDefaults standardUserDefaults] setObject:@(syncConsoleTarget)
                                              forKey:Key_SyncConsoleTarget];
  }
}
...
- (void)setReceivingConfigTime_Class:(NSTimeInterval)receivingConfigTime_Class {
  ...
  [[NSUserDefaults standardUserDefaults]
      setDouble:receivingConfigTime_Class
         forKey:Key_ReceivingConfigTime_Class];
}
...
- (void)setReceivingConfigTime_Color:(NSTimeInterval)receivingConfigTime_Color {
  ...
  [[NSUserDefaults standardUserDefaults]
      setDouble:receivingConfigTime_Color
         forKey:Key_ReceivingConfigTime_Color];
}
...
- (void)_handleFreeRotationDidChange:(LookinMsgActionParams *)param {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:@(boolValue)
                                            forKey:Key_FreeRotation];
}
...
- (void)_handleZInterspaceDidChange:(LookinMsgActionParams *)param {
  ...
  [[NSUserDefaults standardUserDefaults] setObject:@(doubleValue)
                                            forKey:Key_ZInterspace];
}
...
- (BOOL)isSectionShowing:(LookinAttrSectionIdentifier)secID {
  ...
  NSSet<LookinAttrSectionIdentifier> *showingSecIDs =
      [self _showingSecIDsInDefault];
...
- (void)showSection:(LookinAttrSectionIdentifier)secID {
  ...
  [[NSNotificationCenter defaultCenter]
      postNotificationName:NotificationName_DidChangeSectionShowing
                    object:nil];
  [[NSUserDefaults standardUserDefaults]
      setObject:self.storedSectionShowConfig.copy
         forKey:Key_SectionsShow];
}
...
- (NSSet<LookinAttrSectionIdentifier> *)_showingSecIDsInDefault {
  ...
  dispatch_once(&onceToken, ^{
    NSArray<LookinAttrSectionIdentifier> *array = @[
...
+ (BOOL)popupToAskDoubleClickBehaviorIfNeededWithWindow:(NSWindow *)window {
  ...
  alert.messageText = NSLocalizedString(
      @"What do you want to happen when you double click the layer?", nil);
  alert.informativeText = NSLocalizedString(
      @"You can change it at any time in your Preferences.", nil);
  alert.alertStyle = NSAlertStyleInformational;
  [alert
      addButtonWithTitle:NSLocalizedString(@"Expand or collapse layer", nil)];
  [alert addButtonWithTitle:NSLocalizedString(@"Focus on layer", nil)];
  [alert beginSheetModalForWindow:window
                completionHandler:^(NSModalResponse returnCode) {
                  if (returnCode == NSAlertFirstButtonReturn) {
                    // collapse
                    [LKPreferenceManager mainManager].doubleClickBehavior =
...
- (void)reportStatistics {
  [MSACAnalytics
          trackEvent:@"Preference"
      withProperties:@{
        @"DoubleClick" :
            [NSString stringWithFormat:@"%@", @(self.doubleClickBehavior)],
        @"ShowHidden" : [NSString
            stringWithFormat:@"%@", @(self.showHiddenItems.currentBOOLValue)],
        @"RGBA" : [NSString stringWithFormat:@"%@", @(self.rgbaFormat)],
        @"FreeRotation" : [NSString
            stringWithFormat:@"%@", @(self.freeRotation.currentBOOLValue)],
        @"FastMode" :
            [NSString stringWithFormat:@"%@", @(self.fastMode.currentBOOLValue)]
      }];
}
