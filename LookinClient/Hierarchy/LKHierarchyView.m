...
@interface LKHierarchyView () <LKTableViewDelegate, LKTableViewDataSource,
...
- (instancetype)initWithDataSource:(LKHierarchyDataSource *)dataSource {
  if (self = [super initWithFrame:CGRectZero]) {
    ...
    self.backgroundEffectView.blendingMode =
    ...
    [self.guidesShapeLayer
        setLineDashPattern:[NSArray arrayWithObjects:[NSNumber numberWithInt:2],
                                                     [NSNumber numberWithInt:2],
                                                     nil]];
    [self.tableView.contentView.documentView.layer
        addSublayer:self.guidesShapeLayer];

    self.searchTextFieldView = [LKTextFieldView new];
    self.searchTextFieldView.textField.placeholderString =
    ...
    self.searchTextFieldView.textField.lineBreakMode =
    ...
    self.searchTextFieldView.borderColors = LKColorsCombine(
        LookinColorMake(200, 201, 202), LookinColorMake(67, 68, 69));
    ...
    @weakify(self);
    [[[self.searchTextFieldView.textField.rac_textSignal throttle:0.5]
        skip:1] subscribeNext:^(NSString *_Nullable x) {
      @strongify(self);
      x = [x
                                       ...
                                       options:NSRegularExpressionSearch
                                         range:
                                             NSMakeRange(
                                                 0,
                                                 x.length)]; // 去掉所有空白字符
      if ([self.delegate respondsToSelector:@selector(hierarchyView:
                                                didInputSearchString:)]) {
        [self.delegate hierarchyView:self didInputSearchString:x];
      }
    }];
    self.searchTextFieldView.closeButton.target = self;
    self.searchTextFieldView.closeButton.action =
        @selector(_handleSearchCloseButton);

    [[RACObserve(self.tableView, contentInsets) distinctUntilChanged]
    ...
    [RACObserve(dataSource, selectedItem)
        subscribeNext:^(LookinDisplayItem *_Nullable item) {
          @strongify(self);
          [self.tableView reloadDataWithOffset];
          [self scrollToMakeItemVisible:item];
        }];

    [[RACObserve(self.dataSource, hoveredItem) distinctUntilChanged]
        subscribeNext:^(LookinDisplayItem *_Nullable x) {
          @strongify(self);
          [self updateGuidesWithHoveredItem:x];
        }];

    [RACObserve(dataSource, displayingFlatItems)
        subscribeNext:^(NSArray<LookinDisplayItem *> *x) {
          @strongify(self);
    ...
    [[RACObserve(dataSource, displayingFlatItems) throttle:.75]
    ...
    [RACObserve(dataSource, state) subscribeNext:^(NSNumber *x) {
      ...
      if (isFocus &&
          self.searchTextFieldView.textField.stringValue.length > 0) {
...
- (void)renderWithDisplayItems:(NSArray<LookinDisplayItem *> *)displayItems {
  ...
  _minIndentLevel = [[displayItems
      lookin_reduce:^NSNumber *(NSNumber *accumulator, NSUInteger idx,
                                LookinDisplayItem *obj) {
        NSInteger res = accumulator
                            ? MIN(accumulator.integerValue, obj.indentLevel)
                            : obj.indentLevel;
        return @(res);
      }] integerValue];
  ...
  if (displayItems.count == 0) {
    if (!self.emptyDataLabel) {
      ...
      self.emptyDataLabel.stringValue =
          NSLocalizedString(@"No Filter Results", nil);
...
- (void)updateColors {
  [super updateColors];
  self.guidesShapeLayer.strokeColor =
      self.isDarkMode ? LookinColorRGBAMake(255, 255, 255, .3).CGColor
                      : LookinColorRGBAMake(0, 0, 0, .3).CGColor;
}
...
- (LKTableRowView *)tableView:(NSTableView *)tableView
  ...
  LKHierarchyRowView *view = [tableView makeViewWithIdentifier:@"cell"
                                                         owner:self];
  if (!view) {
...
- (void)menuNeedsUpdate:(NSMenu *)menu {
  LKHierarchyRowView *rowView =
      [menu lookin_getBindObjectForKey:kMenuBindKey_RowView];
  ...
  if (!displayItem.isUserCustom) {
    ...
    if (!self.dataSource.isReadOnly) {
      ...
      [menu addItem:({
              NSMenuItem *item = [NSMenuItem new];
              item.title =
                  NSLocalizedString(@"Reload layer and its children", nil);
              if (!isUpdating) {
                item.target = self;
  ...
  if ([displayItem.title hasPrefix:@"UI"] ||
      [displayItem.title hasPrefix:@"CA"]) {
  ...
  NSString *hostViewControllerName =
      displayItem.hostViewControllerObject.lk_simpleDemangledClassName;
  ...
  if (displayItem.displayingObject.ivarTraces.count) {
    NSArray<NSString *> *ivarNames = [[displayItem.displayingObject.ivarTraces
        lookin_map:^id(NSUInteger idx, LookinIvarTrace *value) {
  ...
  [stringsToCopy enumerateObjectsUsingBlock:^(NSString *_Nonnull obj,
                                              NSUInteger idx,
                                              BOOL *_Nonnull stop) {
    if (idx == 0) {
      [menu addItem:[NSMenuItem separatorItem]];
    ...
    if (obj.length == 0) {
      NSAssert(
          NO,
          @"LKHierarchyView, menuNeedsUpdate, stringsToCopy length is zero.");
      return;
    }
    [menu addItem:({
            NSMenuItem *item = [NSMenuItem new];
            ...
            item.title = [NSString
                stringWithFormat:NSLocalizedString(@"Copy text \"%@\"", nil),
                                 obj];
            item;
          })];
  }];
  ...
  if ([displayItem hasPreviewBoxAbility]) {
    if (displayItem.inNoPreviewHierarchy) {
      [menu addItem:({
              ...
              if (displayItem.doNotFetchScreenshotReason ==
  ...
  [menu addItem:({
          NSMenuItem *item = [NSMenuItem new];
          item.target = self;
          item.action = @selector(_handleCopyOid:);
          item.title = NSLocalizedString(@"Copy Oid", nil);
          item;
        })];
}
...
- (void)_handlePrintItem:(NSMenuItem *)menuItem {
  LKHierarchyRowView *view =
      [menuItem.menu lookin_getBindObjectForKey:kMenuBindKey_RowView];
  LookinDisplayItem *item = view.displayItem;
  [[NSNotificationCenter defaultCenter]
      postNotificationName:LKAppShowConsoleNotificationName
                    object:item];
}
...
- (void)_handleReloadSelfAndChildrenItem:(NSMenuItem *)menuItem {
  ...
  [[LKStaticAsyncUpdateManager sharedInstance]
      reloadDisplayItemAndChildren:item];
}
...
- (void)_handleCopyOid:(NSMenuItem *)menuItem {
  LKHierarchyRowView *view =
      [menuItem.menu lookin_getBindObjectForKey:kMenuBindKey_RowView];
  LookinDisplayItem *item = view.displayItem;
  if (!item) {
  ...
  NSString *oidString =
      [NSString stringWithFormat:@"%lu", (unsigned long)item.layerObject.oid];
  NSPasteboard *paste = [NSPasteboard generalPasteboard];
  [paste clearContents];
  [paste writeObjects:@[ oidString ]];
}
...
- (void)_handleExpandRecursively:(NSMenuItem *)menuItem {
  ...
  if ([self.delegate respondsToSelector:@selector
                     (hierarchyView:needToExpandItem:recursively:)]) {
...
- (void)updateGuidesWithHoveredItem:(LookinDisplayItem *)item {
  ...
  NSArray<LookinDisplayItem *> *childrenItems =
      [rootItem.subitems lookin_filter:^BOOL(LookinDisplayItem *obj) {
  ...
  CGFloat rootX = [LKHierarchyRowView
      dislosureMidXWithIndentLevel:rootItem.indentLevel - self.minIndentLevel];
  CGFloat rootY = kRowHeight * rootRow + kRowHeight / 2.0;
  CGFloat rootMaxY =
      [self.displayItems indexOfObject:childrenItems.lastObject] * kRowHeight +
      kRowHeight / 2.0;

  CGMutablePathRef path = CGPathCreateMutable();
  CGPathMoveToPoint(path, NULL, rootX, rootY);
  CGPathAddLineToPoint(path, NULL, rootX, rootMaxY);
  [childrenItems enumerateObjectsUsingBlock:^(LookinDisplayItem *_Nonnull obj,
                                              NSUInteger idx,
                                              BOOL *_Nonnull stop) {
    CGFloat childrenY =
        [self.displayItems indexOfObject:obj] * kRowHeight + kRowHeight / 2.0;
